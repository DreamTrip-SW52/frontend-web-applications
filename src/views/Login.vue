...
const handleLogin = async (e) => {
  ...
  if (!loginErrors?.email?.error && !loginErrors?.password?.error) {
    const { data } = await userApiService.loginWithEmailAndPassword(
      email.value,
      password.value
    );

    if (Array.isArray(data) && data.length === 0) console.log('User not found');
...
const email = ref('patrick@hotmail.com');
