    ...
    <div class="fields">
      <div class="field">
        <div>
          ...
          <InputText @change="resetFormErrors()" v-model="cardNumber" required="true" id="card-number" v-mask="cardMask" placeholder="0000 0000 0000 0000" />
        </div>
      </div>
        ...
        <div>
          ...
          <div class="flex-row expired-date">
            <div class="expired-month">
              <InputText @change="resetFormErrors()" v-model="month" :minlength="2" required="true" id="month" v-mask="monthMask" placeholder="00" />
            </div>
            <div class="expired-year">
              <InputText @change="resetFormErrors()" v-model="year" :minlength="4" required="true" id="year" v-mask="yearMask" placeholder="0000" />
            </div>
          </div>
        </div>
        <div>
          ...
          <div class="flex-row align-items-center security-code" >
            <InputText @change="resetFormErrors()" v-model="cvv" required class="cvv" id="cvv" placeholder="000"
                        v-mask="cvvMask" />
      ...
      <div class="field">
        <div class="errors">
          <small class="p-error" v-for="error of formErrors"> {{ error.message && error?.message }}</small>
        </div>
      </div>
    </div>
    <div class="actions">
...
function onSubmit(){
  ...
  if(props.creditCards.length >= CARD_LIMIT){
    formErrors.value = [{message: 'You have reached the limit of cards to be registered (' + CARD_LIMIT + ')'}]
    return;
  }

  if(!validateForm(cardNumber, month, year, cvv))
    return;

  const creditCard = getStorableCreditCard(cardNumber.value, month.value, year.value, cvv.value);
...
function validateForm(cardNumber, month, year, cvv){
  if(cardNumber.value === '' || month.value === '' || year.value === '' || cvv.value === '')
...
function validateCreditCard(creditCard){
  for(let item of props.creditCards){
    ...
    if(item.cardNumber === creditCard.cardNumber &&
